...
figure_object = figure( ...
    'Visible','off', ...
    'Position',[screen_size(3)/100,screen_size(4)/20,screen_size(3)/1.5,screen_size(4)/1.4], ...
    'Name','Appliance_plot', ...
    'NumberTitle','off', ...
    'MenuBar','figure', ...
...
Spectral_Rolloff_axes=axes('OuterPosition',[2/3,2/3,1/3,1/3], ...
    'Visible','off');

frequency_response_axes=axes('OuterPosition',[1/3,2/3,1/3,1/3], ...
    'Visible','off');
...
figure_object.Visible = 'on';
    ...
    function openclickedcallback(~,~)
        ...
        axes(fft_axes)
        audio_signal_fft = fft(audio_signal);
        abs_fft=abs(audio_signal_fft/number_samples);
        abs_fft_single=abs_fft(1:number_samples/2+1);
        abs_fft_single(2:end-1)=2*abs_fft_single(2:end-1);
        plot(sample_rate*(0:(number_samples/2))/number_samples,abs_fft_single);
        title("Single-Sided Amplitude Spectrum of X(t)")
        xlabel("f (Hz)")
        ylabel("|P1(f)|")

        axes(psd_axes)
        xdft = audio_signal_fft(1:number_samples/2+1);
psdx = (1/(sample_rate*number_samples)) * abs(xdft).^2;
psdx(2:end-1) = 2*psdx(2:end-1);
freq = 0:sample_rate/number_samples:sample_rate/2;
